...
function partial_fit!(rbm::RBM, X,  lr::Real, opt::CDK)
...
function grad_bias!(grad_bias, X, Vhat::Matrix, batch_size)
    suma = fill!(grad_bias, 0.)
    aux = 0 
    @inbounds for i in 1:size(X,1)
        @simd for j in 1:size(X,2)
            grad_bias[i] += X[i,j] - Vhat[i,j]
        end
    end
    grad_bias .= grad_bias./batch_size
end

function compute_grad!(rbm::RBM, X,  opt::CDK)
    ...
    # Correct but generate allocations How can we rewrite it differently
    opt.grad_vis_bias .= squeeze(sum((X .- opt.V_hat), 2),2)./ batch_size;
    opt.grad_hid_bias .= squeeze(sum((opt.H .- opt.H_hat), 2),2)./ batch_size;

    #grad_bias!(opt.grad_vis_bias, X, opt.V_hat, batch_size);
    #grad_bias!(opt.grad_hid_bias, opt.H, opt.H_hat, batch_size);
